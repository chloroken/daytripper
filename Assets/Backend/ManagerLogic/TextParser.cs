...
public static class TextParser
{
    ...
    public static int FindSystemClass(string n, TextAsset[] t)
    {
        ...
        for (var i = 0; i < t.Length; i ++)
        {
            ...
            //Debug.Log("String: " + n + " not found in volume: " + i);
        }
        return(0);
    }
    ...
    public static string GenerateSystemLabel(int c)
    {
        ...
        else if (c == 14) return("Null");
        else if (c == 15) return("Low");
        else return("High");
    }

    public static string ParseSignatureName(string s)
    {
        //Debug.Log("ParseSignatureName: " + s);
        string sigName = "";
        if (s.Contains("Site"))
        {

            // Find the start of the name
            sigName = s.Split(new string[]{"Site"}, StringSplitOptions.None).Last();

            // "Data Site" exception
            if (s.Contains("Data Site"))
            {
                sigName = s.Split(new string[]{"Data Site"}, StringSplitOptions.None).Last();
            }
        
            // Now find the end
            for (var i = 0; i < sigName.Length; i ++)
            {
                if (Char.IsDigit(sigName[i]))
                {
                    sigName = sigName.Substring(0, i);
                    break;
                }
            }
        }
        //Debug.Log("ParseSignatureName: " + sigName.Trim());
        // Purge tabs and spaces
        return(sigName.Trim());
    }
}